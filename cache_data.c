...
cache_region_t CACHE_REGIONS[2048];
...
uint32_t fetch_mem_data(int set,int line,uint32_t address, int offset){
    ...
    if(CACHE_REGIONS[dest].valid = 1 && CACHE_REGIONS[dest].dirty == 1 ){
        uint32_t prefix = (CACHE_REGIONS[dest].tag << 13) | (set << 5);
        if(CACHE_REGIONS[dest].shift == 0){
            uint32_t temp_add, temp_value;
            int i;
            for( i = 0; i < 32; i=i+4){ 
                temp_add = prefix + i; 
                temp_value = 
                (CACHE_REGIONS[dest].cache[i+3] <<24) |
                (CACHE_REGIONS[dest].cache[i+2] <<16) |
                (CACHE_REGIONS[dest].cache[i+1] << 8) |
                (CACHE_REGIONS[dest].cache[i+0] << 0) ;
...
void set_most_recent_data(int set, int dest){
    ...
    CACHE_REGIONS[set+dest].recent = NWAY_DATA - 1;
}
        ...
        if (CACHE_REGIONS[set+i].tag == tag ){
            if(CACHE_REGIONS[set+i].shift == 0){
                if(offset < 0x1D) {
                    ...
                    CACHE_REGIONS[set+i].cache[offset+3] = (value >> 24) & 0xFF;
                    CACHE_REGIONS[set+i].cache[offset+2] = (value >> 16) & 0xFF;
                    CACHE_REGIONS[set+i].cache[offset+1] = (value >> 8) & 0xFF;
                    CACHE_REGIONS[set+i].cache[offset+0] = (value >> 0) & 0xFF;
                    CACHE_REGIONS[set+i].dirty = 1;
                    CACHE_REGIONS[set+i].valid = 1;
                }
                else{
...
uint32_t write_mem(int set,int line,uint32_t address, int offset, uint32_t value){
    ...
    CACHE_REGIONS[dest].valid = 1;
    CACHE_REGIONS[dest].dirty = 1;
    set_most_recent(set,line);
    CACHE_REGIONS[dest].cache[offset+3] = (value >> 24) & 0xFF;
    CACHE_REGIONS[dest].cache[offset+2] = (value >> 16) & 0xFF;
    CACHE_REGIONS[dest].cache[offset+1] = (value >> 8) & 0xFF;
    CACHE_REGIONS[dest].cache[offset+0] = (value >> 0) & 0xFF;
}
